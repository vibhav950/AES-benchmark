...
static inline uint8_t aes_mult(uint8_t a, uint8_t b) {
  return ((((b >> 0) & 1) * xtimes(a, 0)) ^ (((b >> 1) & 1) * xtimes(a, 1)) ^
          ...
          (((b >> 6) & 1) * xtimes(a, 6)) ^ (((b >> 7) & 1) * xtimes(a, 7)));
}
 ...
 * 
 * `d_pt`, `d_ct` reside on device memory.
 * 
 ...
 */
__global__ void AES_Encrypt(uint8_t *d_pt, uint8_t *d_ct, int num_blocks) {
  ...
  state[threadIdx.x] = d_pt[blockIdx.x * AES_BLOCK_SIZE + threadIdx.x];
