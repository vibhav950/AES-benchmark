...
void aesni_do(const char *input_file, const char *output_file,
              aes128_key_t *key, int num_threads, int is_encrypt) {
  FILE *in, *out;
  aes128_ks_t ks;
  long file_size;
  int total_blocks;
  ...
  while (blocks_processed < total_blocks) {
    ...
    for (int i = 0; i < blocks_to_process; i++) {
      if (is_encrypt) {
        aesni_block_encr(input_buffer + i * AES_BLOCK_SIZE,
                        output_buffer + i * AES_BLOCK_SIZE, &ks);
      } else {
        aesni_block_decr(input_buffer + i * AES_BLOCK_SIZE,
                        output_buffer + i * AES_BLOCK_SIZE, &ks);
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "encrypt") == 0) {
    aesni_do(argv[2], argv[3], &key, get_processor_count(), 1);
  } else if (strcmp(argv[1], "decrypt") == 0) {
    aesni_do(argv[2], argv[3], &key, get_processor_count(), 0);
  } else {
